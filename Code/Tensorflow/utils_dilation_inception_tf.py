...
def Dil_Inception1D_tf(input_shape=(256,1),options_dict=None):
    '''
    ...
    tower_1 = Conv1D(op['nfilt'],op['ks1'],padding='same',
                        dilation_rate=op['dif1'],activation=op['act'])(X)
    tower_1 = Conv1D(op['nfilt'],op['ks2'],padding='same',
                        dilation_rate=op['dif2'],activation=op['act'])(tower_1)
    tower_2 = Conv1D(op['nfilt'],op['ks1'],padding='same',
                        dilation_rate=op['dif1'],activation=op['act'])(X)
    tower_2 = Conv1D(op['nfilt'],op['ks3'],padding='same',
                        dilation_rate=op['dif3'], activation=op['act'])(tower_2)
    tower_3 = MaxPooling1D(op['mp1'],strides=op['sl2'],padding='same')(X)
    tower_3 = Conv1D(op['nfilt'],op['ks4'],padding='same',
                    dilation_rate=op['dif4'],activation=op['act'])(tower_3)
    X2 = Concatenate(axis = -1)([tower_1,tower_2,tower_3,X])
    X2 = MaxPooling1D(op['mp1'],strides=op['sl1'])(X2)
    tower_12 = Conv1D(op['nfilt'],op['ks1'],padding='same',
                        dilation_rate=op['dif1'],activation=op['act'])(X2)
    tower_12 = Conv1D(op['nfilt'],op['ks2'],padding='same',
                        dilation_rate=op['dif2'],activation=op['act'])(tower_12)
    tower_22 = Conv1D(op['nfilt'],op['ks1'],padding='same',
                        dilation_rate=op['dif1'],activation=op['act'])(X2)
    tower_22 = Conv1D(op['nfilt'],op['ks3'],padding='same',
                        dilation_rate=op['dif3'],activation=op['act'])(tower_22)
    tower_32 = MaxPooling1D(op['mp1'],strides=1,padding='same')(X2)
    tower_32 = Conv1D(op['nfilt'], op['ks4'],padding='same', 
                        dilation_rate=op['dif4'],activation=op['act'])(tower_32)
    X3 = Concatenate(axis=-1)([tower_12,tower_22,tower_32,X2])
...
def model_Dil_Inception_VAE_tf(options_dict):
    '''
    ...
    data_input = Input(shape=(n_channels,n_timesteps),dtype='float32',
                                                        name='input')
    perchan_model = Dil_Inception1D_tf((n_timesteps,1),options_dict=inp_params)
...
def model_Dil_Inception_DL2_tf(options_dict):
    '''
